...
async def validate_instagram_url(
    ...
    """
    Fast Instagram URL validation - only checks URL format and basic accessibility
    
    This endpoint provides quick validation without metadata extraction
    for responsive frontend validation.
    """
    try:
        ...
        # Fast URL format validation only
        validation_result = await instagram_service.validate_url(instagram_url)
        ...
        # Quick accessibility check (no content extraction)
        try:
            import httpx
            async with httpx.AsyncClient(timeout=5.0) as client:
                # Just check if URL is accessible (HEAD request)
                response = await client.head(instagram_url, follow_redirects=True)
