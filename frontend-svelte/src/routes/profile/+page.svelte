<script>
	...
	import { user, loading, recipes, userProfile } from '$lib/stores/auth.js';
	...
	onMount(() => {
		// Use auth guard to check authentication
		if (!requireAuth()) {
			return;
		}
		loadUserProfile();
		...
		{#if $user}
			...
			<div class="bg-white rounded-lg shadow-sm p-6 mb-8">
				<div class="flex items-center space-x-6">
					<!-- Profile Picture with Safe Loading -->
					<SafeImage 
						src={$user.photoURL} 
					...
					<div class="flex-1">
						...
						<!-- Social Login Provider Information -->
						{#if $userProfile.providerData && $userProfile.providerData.length > 0}
							<div class="flex items-center mt-2 space-x-2">
								<span class="text-sm text-gray-500">Signed in with:</span>
								{#each $userProfile.providerData as provider}
						...
						<p class="text-sm text-gray-500 mt-1">
							Member since {formatDate($userProfile.metadata?.creationTime)}
						</p>
					</div>
